...
from models.hiera.hiera_utils import (
    pretrained_model,
    conv_nd,
    do_pool,
    do_masked_conv,
...
from models.hiera.hfhub import has_config, PyTorchModelHubMixin
...
class MaskUnitAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        cls_qkv = qkv_full[:, : self.num_cls_tokens, :]
        qkv = qkv_full[:, self.num_cls_tokens :, :]
        qkv = qkv.reshape(B, -1, num_windows, 3, self.heads, self.head_dim).permute(
            3, 0, 4, 2, 1, 5
        )
        ...
        # TODO: add class tokens here
        cls_qkv = (
            cls_qkv.reshape(B, -1, 1, 3, self.heads, self.head_dim)
            .expand(-1, -1, num_windows, -1, -1, -1)
            .permute(3, 0, 4, 2, 1, 5)
        )
        cls_q, _, _ = cls_qkv[0], cls_qkv[1], cls_qkv[2]

        q_full = torch.cat([cls_q, q], dim=-2)

        if hasattr(F, "scaled_dot_product_attention"):
            # Note: the original paper did *not* use SDPA, it's a free boost!
            x = F.scaled_dot_product_attention(q_full, k, v)
        else:
            attn = (q_full * self.scale) @ k.transpose(-1, -2)
            attn = attn.softmax(dim=-1)
            x = attn @ v
        cls_x = x[:, :, :, : self.num_cls_tokens, :]
        original_x = x[:, :, :, self.num_cls_tokens :, :]

        original_x = original_x.transpose(1, 3).reshape(B, -1, self.dim_out)

        # NOTE: DOING MEAN AS IT IS SIMPLE - I THINK IT IS THE NUMBER OF "WINDOWS"
        cls_x = (
            cls_x.mean(axis=2, keepdim=True)
            .transpose(1, 3)
            .reshape(B, -1, self.dim_out)
        )
        x = torch.cat([cls_x, original_x], dim=-2)
        x = self.proj(x)
        return x
...
class Hiera(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        if num_cls_tokens and num_cls_tokens > 0:
            self.num_cls_tokens = num_cls_tokens
    ...
    def get_pos_embed(self, s_shape=None) -> torch.Tensor:
        if self.sep_pos_embed:
            return self.pos_embed_spatial.repeat(
                1, self.tokens_spatial_shape[0], 1
            ) + torch.repeat_interleave(
                self.pos_embed_temporal,
                # F.interpolate(self.pos_embed_temporal.permute(0,2,1),32).permute(0,2,1),
                self.tokens_spatial_shape[1] * self.tokens_spatial_shape[2],
                dim=1,
            )
        else:
    ...
    def forward(
        ...
        x = x + self.get_pos_embed(x.shape)
        x = self.unroll(x)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if return_intermediates and i in self.stage_ends:
                intermediates.append(
                    self.reroll(x[:, self.num_cls_tokens :], i, mask=mask)
                )
                intermediate_cls_tokens.append(x[:, 0])
                # print("SAVED:", i, x[:, 0].shape)
        ...
        if return_intermediates:
            return x, intermediates, intermediate_cls_tokens
